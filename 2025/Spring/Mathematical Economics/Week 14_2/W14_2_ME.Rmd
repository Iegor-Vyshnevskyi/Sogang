...
![Sweeping lines for different c](p8.png)
...
constraint_color_1 <- "#1f77b4" # Blue
constraint_color_2 <- "#d62728" # Red

ggplot(df, aes(x = x)) +
  geom_line(aes(y = y1), color = constraint_color_1) +
  geom_line(aes(y = y2), color = constraint_color_2) +
  annotate("text", x = 2, y = 2.5, label = "x + y <= 4", color = constraint_color_1) +
  annotate("text", x = 1, y = 5, label = "2x + y <= 6", color = constraint_color_2) +
  coord_cartesian(xlim = c(0, 6), ylim = c(0, 6)) +
  labs(title = "Constraint Lines", x = "x", y = "y") +
  theme_minimal()
```
